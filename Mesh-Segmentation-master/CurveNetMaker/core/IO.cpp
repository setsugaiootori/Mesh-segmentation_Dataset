...
void MeshSegment::saveSegmentation()
{
	...
	//face label
	QString FaceFile = m_filename;
	FaceFile.replace(FaceFile.lastIndexOf(".") + 1, 3, "label");
	convertVertexToFaceLabelling(vertexLabel, faceLabel);
	File.open(FaceFile.toStdString(), std::ios::out);
	if (!File.good())
		return;

	File << "[num of faces] [num of clusters]" << endl;
	File << myMesh->getFaces().size() << ' ' << *std::max_element(faceLabel.begin(), faceLabel.end()) + 1 << endl;
	File << "[index of faces] [label of label]" << endl;
	for (MyMesh::FaceIter f_it = myMesh->getFaces().begin(); f_it != myMesh->getFaces().end(); f_it++)
	{
		File << "f" << f_it->id() << ' ' << faceLabel[f_it->id()] << endl;
	}
	File.close();
